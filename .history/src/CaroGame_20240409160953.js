...
const CaroGame = ({ navigation }) => {
  ...
  const getBestMove = (currentBoard, currentPlayer, alpha, beta, depth) => {
    ...
    // Kiểm tra nếu có nước đi có thể chặn đối thủ
    for (let i = 0; i < ROWS; i++) {
      for (let j = 0; j < COLS; j++) {
        if (currentBoard[i][j] === EMPTY) {
          ...
          if (score === 10) {
            return [i, j]; // Trả về nước đi có thể chặn đối thủ ngay lập tức nếu có
          }
    ...
    // Tiếp tục tìm nước đi tốt nhất nếu không có nước đi có thể chặn đối thủ
    for (let i = 0; i < ROWS; i++) {
      for (let j = 0; j < COLS; j++) {
        if (currentBoard[i][j] === EMPTY) {
    ...
    // Chọn một trong các nước đi tốt nhất ngẫu nhiên nếu không tìm thấy nước đi chặn đối thủ
