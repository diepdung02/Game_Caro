...
const CaroGame = ({ navigation }) => {
  ...
  const getBestMove = (currentBoard, currentPlayer, alpha, beta, depth) => {
    ...
    // Nếu không tìm thấy nước đi có thể chặn đối thủ, thử các nước đi khác trong danh sách bestMoves
    let randomBlockingMove = [];
    for (let move of bestMoves) {
      const [row, col] = move;
      currentBoard[row][col] = opponent;
      let score = evaluateBoard(currentBoard);
      if (score === 10) {
        randomBlockingMove = move;
        break;
      }
      currentBoard[row][col] = EMPTY;
    }
  
    if (randomBlockingMove.length > 0) {
      return randomBlockingMove;
    }
  
    // Nếu không tìm thấy nước đi có thể chặn đối thủ, chọn một trong các nước đi tốt nhất ngẫu nhiên
    const randomMove =
