...
const CaroGame = ({ navigation }) => {
  ...
  const getBestMove = (currentBoard, currentPlayer, alpha, beta, depth) => {
    ...
    // Sau đó ưu tiên nước đi có thể thắng
    for (let i = 0; i < ROWS; i++) {
      for (let j = 0; j < COLS; j++) {
        if (currentBoard[i][j] === EMPTY) {
    ...
    // Nếu không thể chiến thắng trong nước đi hiện tại của đối thủ hoặc chính người chơi, thực hiện tìm nước đi tốt nhất cho chính người chơi
    for (let i = 0; i < ROWS; i++) {
      for (let j = 0; j < COLS; j++) {
        if (currentBoard[i][j] === EMPTY) {
          currentBoard[i][j] = currentPlayer;
    ...
    // Chọn một trong các nước đi tốt nhất ngẫu nhiên nếu không tìm thấy nước đi chặn đối thủ hoặc nước đi có thể thắng
