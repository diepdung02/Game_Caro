    ...
    return board.map((row, rowIndex) => (
      <View key={rowIndex} style={styles.row}>
        {row.map((cell, colIndex) => (
          <
          key={colIndex}
          style={[styles.cell, {Color: cell === PLAYER_X ? 'green' : cell === PLAYER_O ? 'blue' : 'white'}]}
          onPress={() => handleClick(rowIndex, colIndex)}>
           <Text style={[styles.cellText, {color: cell === PLAYER_X ? 'red' : 'black'}]}>{cell}</Text>
